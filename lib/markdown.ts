...
/**
 * 매우 심플한 Markdown → HTML 변환기 (의존성 없이 동작)
 * 지원: #/##/###, - 리스트, **bold**, [text](url), 코드블록(```), 인라인코드(``)
 */
export function markdownToHtml(md: string) {
  // 먼저 HTML 이스케이프해서 원본 HTML 주입을 막음
  let s = escapeHtml(md);
  ...
  // 문단 처리: 빈 줄 기준으로 나눠서 <p>
  const parts = s.split(/\n{2,}/).map((p) => p.trim()).filter(Boolean);
  s = parts
    .map((p) => {
      // ul/pre/h1~h3 로 시작하면 그대로
      if (/^<h[1-3]\b|^<ul\b|^<pre\b/.test(p)) return p;
...
function parseFrontmatter(raw: string) {
  // --- ... --- 형태만 지원
  if (!raw.startsWith("---")) {
    return { meta: {} as DocMeta, content: raw.trim() };
    }
  const end = raw.indexOf("\n---", 3);
  if (end === -1) return { meta: {} as DocMeta, content: raw.trim() };
